...
class DataProcessor:
    ...
    def export_json(self, json_path='data/modified/movies.json'):
        """Export the processed DataFrame to a JSON file for frontend use."""
        if self.df is not None:
            self.df.to_json(json_path, orient='records', force_ascii=False)
...
def main():
    ...
    processor.save()
    processor.export_json('data/modified/movies.json')
